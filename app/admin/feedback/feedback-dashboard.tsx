...
const broadcastTemplates = [
  {
    ...
    message: "Hi {firstName}! Thanks for being part of Keep Swimmin’. I’m always working to improve the app and make your experience better. 💙🐳",
    actionURL: "",
  },
  {
    label: "New update",
  ...
  {
    label: "Feedback Hub",
    message: "Hey everyone, Denis here! Have an idea or found a problem? Open Keep Swimmin’ and visit Settings > Feedback Hub. I’ll let you know when it’s fixed!",
    actionURL: "",
  },
  {
    label: "Thank you",
...
export function FeedbackDashboard() {
  ...
  function toggleSidebar() {
    setSidebarOpen((current) => {
      const next = !current;
      window.localStorage.setItem("feedback-hub-sidebar", next ? "open" : "closed");
      return next;
    });
  }
  ...
  return (
    <main className={`feedback-admin fixed inset-0 z-50 flex overflow-hidden bg-[#f7f9fc] text-slate-900 ${darkTheme ? "dark-mode" : ""}`}>
      ...
      {broadcastOpen && <div className="absolute inset-0 z-50 grid place-items-center bg-slate-950/30 p-6 backdrop-blur-sm"><form onSubmit={sendBroadcast} className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#5285f7]">Notification composer</p><h2 className="mt-1 text-xl font-bold">Send a notification</h2><p className="mt-2 text-xs leading-5 text-slate-500">Choose recipients, write your message, and review exactly what they will receive.</p></div><button type="button" onClick={() => setBroadcastOpen(false)} aria-label="Close" className="rounded-xl p-2 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Recipients</p><p className="mt-1 text-sm font-semibold text-slate-700">{selectedAudience.length ? `${selectedAudience.length} selected` : "No one selected"}</p>{selectedAudience.length > 0 && <p className="mt-1 text-[11px] text-slate-500">{selectedAudience.slice(0, 3).map((member) => member.displayName || member.email).join(", ")}{selectedAudience.length > 3 ? ` +${selectedAudience.length - 3} more` : ""}</p>}</div><button type="button" onClick={() => setAudienceOpen(true)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-semibold text-[#5285f7]">{selectedAudience.length ? "Review recipients" : "Choose recipients"}</button></div></div><div className="mt-5"><div className="flex items-center gap-2 text-xs font-semibold text-slate-600"><BookmarkPlus className="h-3.5 w-3.5 text-[#5285f7]" />Saved messages</div><div className="mt-2 flex flex-wrap gap-2">{broadcastTemplates.map((template) => <button type="button" key={template.label} onClick={() => setBroadcastBody(template.message)} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] font-semibold text-slate-600 transition hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700">{template.label}</button>)}</div></div><label className="mt-5 block text-xs font-semibold text-slate-600">Message<textarea required autoFocus maxLength={broadcastBody.includes("{firstName}") ? 135 : 150} value={broadcastBody} onChange={(event) => setBroadcastBody(event.target.value)} placeholder="Write your message, or use {firstName} for a personal greeting…" className="mt-2 h-40 w-full resize-none rounded-xl border border-slate-200 p-3 text-sm leading-6 outline-none focus:border-blue-300 focus:ring-4 focus:ring-blue-50" /></label><div className="mt-1 flex items-center justify-between text-[10px] text-slate-400"><span>{broadcastBody.includes("{firstName}") ? "Each user’s first name will be added automatically." : "Use {firstName} to personalize this message."}</span><span>{broadcastBody.length}/{broadcastBody.includes("{firstName}") ? 135 : 150}</span></div><div className="mt-4"><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">Preview{selectedAudience[0] ? ` for ${selectedAudience[0].displayName || selectedAudience[0].email}` : ""}</p><div className="notification-preview flex items-start gap-3 rounded-2xl border p-3.5 shadow-sm"><img src="/keep-swimmin-app-icon.png" alt="Keep Swimmin’ app icon" className="h-10 w-10 shrink-0 rounded-[9px]" /><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3"><p className="notification-preview-app truncate text-[11px] font-semibold">KEEP SWIMMIN’</p><span className="notification-preview-time shrink-0 text-[11px]">now</span></div><p className="notification-preview-body mt-1 whitespace-pre-wrap text-xs leading-5">{previewBroadcastBody.trim() || "Your message will appear here."}</p></div></div></div>{broadcastResult && <p className="mt-4 rounded-xl bg-emerald-50 p-3 text-xs font-semibold text-emerald-700">{broadcastResult}</p>}<div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setBroadcastOpen(false)} className="rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-500 hover:bg-slate-50">Close</button><button disabled={!selectedAudience.length || !broadcastBody.trim() || broadcastBody.length > (broadcastBody.includes("{firstName}") ? 135 : 150) || sendingBroadcast} className="flex items-center gap-2 rounded-xl bg-[#5285f7] px-5 py-2.5 text-xs font-semibold text-white shadow-lg shadow-blue-200 disabled:cursor-not-allowed disabled:opacity-45"><Send className="h-3.5 w-3.5" />{sendingBroadcast ? "Sending…" : `Review and send${selectedAudience.length ? ` to ${selectedAudience.length}` : ""}`}</button></div></form></div>}
